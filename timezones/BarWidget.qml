...
Rectangle {
    ...
    readonly property string tipTimeFormat: cfg.tipTimeFormat || defaults.tipTimeFormat || "HH:mm, ddd DD-MMM"
    ...
    function updateTime() {
        if (enabledTimezones.length === 0) {
            currentCity = pluginApi?.tr("timezones.no-timezone");
            currentTime = "--:--";
            return;
        }
    ...
    function updateTooltip() {
        if (timezones.length === 0)
            return pluginApi?.tr("timezones.configure") || "Configure timezones";
        let str = "<table>";
        for (let i = 0; i < timezones.length; i++) {
            let tz = timezones[i];
            str += `<tr><td>${tz.name}</td><td>${TimezoneUtils.getTimeInTimezone(tz.timezone, tipTimeFormat)}</td></tr>`;
        }
        str += "</table>";
        return `${str}\n${pluginApi?.tr("timezones.tooltip.click") || "Click to configure"}`;
    }
    ...
    readonly property string displayText: {
        if (enabledTimezones.length === 0)
            return pluginApi?.tr("timezones.no-timezone");
